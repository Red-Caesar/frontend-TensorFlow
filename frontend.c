...
//   Try to apply the window - if it fails, return and wait for more data.
    metrics(samples, num_samples, "First input");
   if (!WindowProcessSamples(&state->window, samples, num_samples,
                             num_samples_read)) {
     return output;
   }
   metrics(state->window.output, state->window.size, "After WindowProcessSamples()");
  ...
  // FFT can have as much resolution as possible).
   ...
   metrics(state->fft.output, state->fft.input_size, "After FftCompute()");
//   We can re-ruse the fft's output buffer to hold the energy.
   ...
   metrics(energy, state->fft.input_size, "After FilterbankConvertFftComplexToEnergy()");
   ...
   metrics(state->filterbank.work, state->filterbank.num_channels, "After FilterbankSqrt()");
//   Apply noise reduction.
   NoiseReductionApply(&state->noise_reduction, scaled_filterbank);

   metrics(state->filterbank.work, state->filterbank.num_channels, "After NoiseReductionApply()");

   if (state->pcan_gain_control.enable_pcan) {
     PcanGainControlApply(&state->pcan_gain_control, scaled_filterbank);

     metrics(state->filterbank.work, state->filterbank.num_channels, "After PcanGainControlApply()");
   }
...
//   Apply the log and scale.
   ...
   metrics(state->filterbank.work, state->filterbank.num_channels, "After LogScaleApply()");
